        ...
        <form class="space-y-4 md:space-y-6" on:submit|preventDefault={submitForm}>
          <div>
            ...
            <input 
              ...
              id="password" 
              placeholder="••••••••" 
              class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-emerald-600 focus:outline-none block w-full py-2.5 px-3"
              required
            >
            <button type="button" on:click={generatePassword} class="text-xs text-gray-500 mt-1">Generate Password</button>
          </div>
          <div>
          ...
          <p class="text-sm font-light text-gray-500">
            Ingat password Anda? <a href="/login" use:inertia class="font-medium text-emerald-600 hover:underline">Login disini</a>
          </p>
        </form>
      </div>
    </div>
